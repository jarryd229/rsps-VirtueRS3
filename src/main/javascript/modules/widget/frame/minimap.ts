...
import { setVarp, varbit, setVarBit, varp, setVarc } from 'engine/var';

import { defaultHandler } from 'shared/util';
import { openCentralWidget, openWidget, hideWidget } from 'shared/widget';
import { sendDebugMessage } from 'shared/chat';
import { runAnim } from 'shared/anim';
import { examineMoneyPouch, requestWithdrawCoins } from 'shared/inv/money-pouch';
 ...
 * @since 05/03/2016
 */
_events.bindEventListener(EventType.IF_BUTTON, 1465, (ctx) => {
	...
	switch (ctx.component) {
		...
		case 22://Money pouch options
			switch (ctx.button) {
				case 1://Toggle money pouch
					...
					setVarBit(player, 1192, wasOpen ? 0 : 1);
					return;
		...
		case 44://World map
			if (ctx.button == 3) {
				...
				setVarp(player, 3926, 0);
				setVarp(player, 3928, -1);
				setVarp(player, 3929, -1);
				openWidget(player, 1477, 496, 669, true);
				setVarc(player, 3838, 0);
				setVarc(player, 3840, 0);
				hideWidget(player, 1477, 20, true);
				hideWidget(player, 1477, 21, true);
				hideWidget(player, 1477, 22, true);
				hideWidget(player, 1477, 506, true);
				hideWidget(player, 1477, 388, true);
				...
				openWidget(player, 1477, 16, 1421, true);
				openWidget(player, 1477, 15, 1422, false);
				openWidget(player, 1422, 107, 698, true);
